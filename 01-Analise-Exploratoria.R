...
summary(carros$ano) #Variaveis que não deve ser numerica

carros$ano <- as.factor(carros$ano)
carros$cor <- as.factor(carros$cor)
...
##### Análise Exploratória de Dados Para Variáveis Categoricas ##### 
...
plot(x = carros$kilometragem, y = carros$preco,
     ...
     xlab = "Kilometragem",
     ylab = "Preço (R$)")
abline(lm(carros$preco ~ carros$kilometragem), col = "red", lwd = 1)
...
table(carros$ano, carros$transmissao)

table(carros$ano, carros$modelo)

table(carros$modelo, carros$transmissao)

table(carros$ano, carros$cor)

table(carros$modelo, carros$cor)

table(carros$transmissao, carros$cor)
...
carros$conserv <- carros$cor %in% c("Preto", "Cinza", "Prata", "Branco") # Cores definidas por uma Analista de Negocio (Retorna TRUE ou FALSE)
head(carros)
