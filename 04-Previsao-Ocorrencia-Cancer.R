...
# Detectamos um problema de escala entre os dados, que então precisam ser padronizados
# O cálculo de distância feito pelo kNN é dependente das medidas de escala nos dados de entrada.
summary(dados[c("radius_mean", "area_mean", "smoothness_mean")])

# Criando um função de padronizacao
padronizar <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

# Testando a função de normalização - os resultados devem ser idênticos
padronizar(c(1, 2, 3, 4, 5))
padronizar(c(10, 20, 30, 40, 50))

# Normalizando os dados
dados_padr <- as.data.frame(lapply(dados[2:31], padronizar))
View(dados_padr)
...
dados_treino <- dados_padr[1:469, ]
dados_teste <- dados_padr[470:569, ]

# Criando os labels para os dados de treino e de teste
dados_treino_labels <- dados[1:469, 1]
dados_teste_labels <- dados[470:569, 1]
length(dados_treino_labels) / dim(dados)[1]
length(dados_teste_labels) / dim(dados)[1]
...
?CrossTable
CrossTable(x = dados_teste_labels, y = modelo_knn_v1, prop.chisq = FALSE, digits = 2)
...
# Benigno -> Negativa (para ocorrência de câncer de mama)
# Maligno -> Positiva (para ocorrência de câncer de mama)

# Temos:
# Cenário 1: Célula Benigno (Observado) x Benigno (Previsto) - 61 casos - true negative 
# Cenário 2: Célula Maligno (Observado) x Benigno (Previsto) - 02 casos - false negative (o modelo errou)
# Cenário 3: Célula Benigno (Observado) x Maligno (Previsto) - 00 casos - false positive (o modelo errou)
# Cenário 4: Célula Maligno (Observado) x Maligno (Previsto) - 37 casos - true positive 
...
# Usando a função scale() para normalizar o z-score 
?scale()
...
# Criando uma tabela cruzada dos dados previstos x dados atuais
CrossTable(x = dados_teste_labels, y = modelo_knn_v2, prop.chisq = FALSE, digits = 2)
...
?runif
dados[,'index'] <- ifelse(runif(nrow(dados)) < 0.8,1,0) #forma de dividir um dataset em treino e teste
